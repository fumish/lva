...
input.preprocessing.add.bias <- function(x){
  structure.info <- dim(x)
  if(is.null(structure.info)){
    n <- length(x)
    M <- 1
  }else if(length(structure.info) == 2){
    n <- dim(x)[1]
    M <- dim(x)[2]
  }else if(length(structure.info) == 3){
    n <- dim(x)[2]
    M <- dim(x)[3]
  }else{
    print("This is illeagal argument !!")
    return(NULL)
  }
  
  processed.x <- matrix(0, nrow = n, ncol=(M+1))
  processed.x[,1] <- rep(1, n)
  
  processed.x[,2:(M+1)] <- as.matrix(x)
  return(processed.x)
}

input.preprocessing.quad <- function(x){
  structure.info <- dim(x)
  if(is.null(structure.info)){
    n <- length(x)
    M <- 1
  ...
  processed.x <- matrix(0, nrow = n, ncol=(2*M+1))
  
  processed.x[,1] <- rep(1, n)
  processed.x[,2:(M+1)] <- as.matrix(x)
  processed.x[,(M+2):(2*M+1)] <- as.matrix(x)^2
  return(processed.x)
}

# generate.param.logistic.mixture <- function(seed = 1, K, M, sigma = 1){
#   set.seed(seed = seed)
#   ratio <- round(rdirichlet(rep(1,K)),digits = 2)
#   ratio[K] <- 1-sum(ratio[1:(K-1)])
#   weight <- round(matrix(sigma*rnorm(M*K), nrow=M, ncol=K), digits = 2)
...
rlogistic.mixture <- function(n, base.dim, x.range, input.preprocessor, param, seed = 1, is.label=TRUE){
  set.seed(seed=seed)
  
  ##input
  x.base <- matrix(runif(n*M, min = x.range[1], max = x.range[2]), nrow=n, ncol=base.dim)
  x <- input.preprocessor(x.base)
...
initialize.logistic.mixture.fast <- function(n, K, M, init.Sigma,
...
initialize.logistic.mixture <- function(n, K, M, init.Sigma,
  ...
  mu <- matrix(0, nrow=M, ncol=K)
  for(k in 1:K){
    inv.beta[,,k] <- solve(beta[,,k])
    mu[,k] <- mvrnorm(n=1, mu=rep(0,M), Sigma=inv.beta[,,k])
  }
  ...
  return(list(param = list(phi=phi, mu=mu, beta=beta),
              latent = list(latent.variable=est.latent.variable),
              auxilirary = list(auxilirary.variable=est.auxilirary.variable)))
}

lva.free.energy.fast <- function(x, in.out.matrix, prior.hyperparameter,
...
lva.update.parameters.fast <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
...
lva.update.latent.variable.fast <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
  ##pre-processing for this function
  n <- dim(x)[1]
  M <- dim(x)[2]
...
lva.update.auxilirary.variable.fast <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
  ##pre-processing for this function
  n <- dim(x)[1]
...
lva.update.parameters <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
  ...
  est.phi <- apply(est.latent.variable,2,sum)+phi
  for(k in 1:K){
    extend.est.g.eta <- matrix(rep(est.g.eta[,k],M),nrow=n,ncol=M)
    extend.est.latent.variable <- matrix(rep(est.latent.variable[,k],M), nrow=n, ncol=M)
    est.beta[,,k] <- t(x) %*% (extend.est.latent.variable * tanh(sqrt(extend.est.g.eta)/2) / (2*sqrt(extend.est.g.eta)) * x) + beta*diag(M)
    est.mu[,k] <- solve(est.beta[,,k]) %*% 
      apply((extend.est.latent.variable * in.out.matrix),2,sum)
  }
  
  return(list(name="param", updated.val=list(phi=est.phi, beta=est.beta, mu=est.mu)))
}
...
lva.update.latent.variable <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
  ...
  for(k in 1:K){
    est.h.xi[,k] <- digamma(est.phi[k]) - digamma(sum(est.phi)) +
      in.out.matrix %*% est.mu[,k] - log(2*cosh(sqrt(est.g.eta[,k])/2)) -
      tanh(sqrt(est.g.eta[,k])/2)/(4*sqrt(est.g.eta[,k]))*(diag(x %*% (solve(est.beta[,,k]) + est.mu[,k] %*% t(est.mu[,k])) %*% t(x)) - est.g.eta[,k])
    # est.h.xi[,k] <- digamma(est.phi[k]) - digamma(sum(est.phi)) +
    #   in.out.matrix %*% est.m[,k] - log(2*cosh(sqrt(est.g.eta[,k])/2))
  }
  max.est.h.xi <- apply(est.h.xi,1,max)
  norm.est.h.xi <- est.h.xi - max.est.h.xi
  est.latent.variable <-  exp(norm.est.h.xi)/matrix(rep(apply(exp(norm.est.h.xi),1,sum),K),nrow=n,ncol=K)
...
lva.update.auxilirary.variable <- function(x, in.out.matrix, K, prior.hyperparameter, current.est.result){
  ...
  est.g.eta <- matrix(0, nrow=n, ncol=K)
  for(k in 1:K){
    est.g.eta[,k] <- diag(x %*% (solve(est.beta[,,k]) + est.mu[,k] %*% t(est.mu[,k])) %*% t(x))
  }
  
  return(list(name="auxilirary", updated.val=list(auxilirary.variable=est.g.eta)))
}

prediction.logistic.mixture <- function(x, parameter.num=10000, estimation.result){
  library(MASS)
...
prediction.logistic.mixture.fast <- function(x, estimation.result){
  ratio <- estimation.result$mean.estimator$ratio
  n <- dim(x)[1]
  M <- dim(x)[2]
  K <- length(ratio)
...
calc.accuracy <- function(test.y, prob.list, target.number){
  if(target.number > length(prob.list)){
    target.number <- length(prob.list)
  }
  
  priority.list <- order(prob.list, decreasing = T)
  
  pred.y.all <- rep(NA, length(prob.list))
  pred.y.all[priority.list[1:target.number]] <- runif(target.number) < prob.list[priority.list[1:target.number]]
  TP.TN <- sum(as.numeric(pred.y.all[priority.list[1:target.number]] == test.y[priority.list[1:target.number]]))
  accuracy <- TP.TN / target.number
  return(accuracy)
  
}

calc.gs.recall <- function(test.y, prob.list, accum.range){
  ## searching set
  priority.list <- order(prob.list, decreasing = T)
  
  total.true <- sum(test.y)
  
    # accum.range <- seq(from = 1, to = target.number, by = 10)
  accum.true <- numeric(length(accum.range))
  for(i in 1:length(accum.range)){
    accum.true[i] <- sum(test.y[priority.list[1:accum.range[i]]])
  }

  return(list(accum.true = accum.true, recall = accum.true/ total.true))
  
}

calc.gs.precision <- function(test.y, prob.list, accum.range){
  ## searching set
  priority.list <- order(prob.list, decreasing = T)
  
  total_true <- sum(test.y)
  # accum.range <- seq(from = 1, to = target.number, by = 10)
  accum.true <- numeric(length(accum.range))
  for(i in 1:length(accum.range)){
    accum.true[i] <- sum(test.y[priority.list[1:accum.range[i]]])
  }
  return(list(accum.true = accum.true, precision = accum.true / accum.range))
}

calc.F.measure <- function(recall, precision){
  return(2/(1/recall + 1/precision))
}

transform.zscore <- function(x, target.ind){
  n <- dim(x)[1]
  mean.x <- apply(x,2,mean)
  std.x <- sqrt(apply(x,2,var)*(n-1)/n) #transform unbised variance to sample variance
  processed.x <- (x[,target.ind] - matrix(rep(mean.x[target.ind],n),nrow=n, ncol=length(target.ind), byrow = T)) / 
    matrix(rep(std.x[target.ind],n),nrow=n, ncol=length(target.ind), byrow=T)
  return(processed.x)
}

transform.max.min <- function(x, target.ind, max = 1, min=0){
  n <- dim(x)[1]
  max.x <- as.numeric(apply(x[,target.ind],2,max))
  min.x <- as.numeric(apply(x[,target.ind],2,min))
  processed.x <- (x[,target.ind]-matrix(rep(min.x,n),nrow=n, ncol=length(target.ind), byrow = T))/
    (matrix(rep(max.x - min.x,n),nrow=n, ncol=length(target.ind), byrow = T))*(max-min)+min
  return(processed.x)
}

transform.logsacle <- function(x, target.ind, base=10, exp.lower.lim = 1){
  return(log(x[,target.ind] + exp.lower.lim, base = base))
}

lva.minibatch.estimation.learning <- function(total.size, batch.size,
                                              K, M,
  ...
  for(ite1 in 1:iteration){
    ...
    if(ite1 < iteration){
      ...
      # current.training.x[,4] <- current.training.x[,2]^3
...
lmm.em <- function(x,y,K,
  ...
  for(res in 1:restart){
    ...
    # candidate.z.xi <- matrix(0, nrow=n, ncol=K)
    # for(i in 1:n){
    #   candidate.z.xi[i,] <- rdirichlet(rep(1,K))
    # }
    ...
    for(ite in 1:iteration){
      ...
      # for(k in 1:K){
      #   k.logistic.data <- data.frame(x = x, y  = candidate.z.xi[,k] * y)
      #   k.glm.result <- glm(y~.-1, data=k.logistic.data,family=quasibinomial(link="logit") )
      #   k.glm.coef <- as.numeric(coef(k.glm.result))
      #   # k.glm.coef <- lmm.newton(x,y,candidate.z.xi[,k], candidate.z.xi)
      #   k.glm.coef[is.na(k.glm.coef)] <- 0
      #   candidate.weight[,k] <- k.glm.coef
      # }
...
lmm.newton <- function(x,y,est.z.xi, iteration=100, tol=1e-5, seed=NA){
  M <- ncol(x)
  n <- nrow(x)
  weight <- rnorm(M)
  if(!is.na(seed)){
  ...
  for(ite in 1:iteration){
    r <- 1/(1+exp(-x%*%weight))
    Hessian <- t(x) %*% (matrix(rep(est.z.xi * r * (1-r),M), nrow=n, ncol=M)*x)
    grad <- t(est.z.xi * (r-y)) %*% x
    
    if(det(Hessian) < tol){
      print("Hessian is singular, so we break this function")
      break
    }
    
    weight <- weight - solve(Hessian) %*% t(grad)
